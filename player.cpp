...
Player::Player() {
    ...
    collisionRadius = 60; // Bán kính phù hợp với hình ảnh Kong
}
...
void Player::init(SDL_Texture* runTexture, SDL_Texture* slideTex) {
    ...
    sprite.frameDelayMax = 5;
    slideSprite.frameDelayMax = 5; // Tốc độ animation trượt
}
...
void Player::update(float deltaTime, const vector<Obstacle>& obstacles){
    ...
    for (const auto& obstacle : obstacles) {
        if (checkPlatformCollision(obstacle.rect)) {
            onAnyGround = true;
            platformSpeed = obstacle.speed;
            break;
        }
    }

    // Nếu đứng trên chướng ngại vật di chuyển, cập nhật vị trí X của người chơi
    if (onAnyGround && platformSpeed != 0.0f) {
        // Di chuyển người chơi cùng với chướng ngại vật
        x -= platformSpeed;
    }
...
SDL_Point Player::getCollisionCenter() const {
    ...
    center.x = static_cast<int>(x + 45); // tâm cách mép trái khoảng x=50
    center.y = static_cast<int>(y + 82); // tâm cách mép trên của ảnh khoảng y=85
    return center;
}
